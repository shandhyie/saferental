...
if ( isset($_POST['username']) && isset($_POST['password']) ) {
    ...
    $sql_check = "SELECT nama_lgkp, level_user, id_user FROM users WHERE username=? AND password=? LIMIT 1";

    $check_log = $dbconnect->prepare($sql_check);
    $check_log->bind_param('', $username, $password);
    ...
    if ( $check_log->num_rows == 1 ) {
        $check_log->bind_result($nama_lgkp, $level_user, $id_user);
        ...
        header('location:member/'.$level_user);
        exit();
